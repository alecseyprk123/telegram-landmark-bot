    ...
    for model_name in MODELS:
        ...
        try:
            response = client.models.generate_content(
                model=model_name,
                contents=[
                    "Назови одним-двумя словами, что изображено на этом фото. Затем напиши краткую информацию.Так же если на фотке нету не какой достопримечательности напиши данно сообщение (просто это сообщение не нужно расписывать что изоброено на картинке просто этот текст и все): На данном фото отсутсвует достопримечательность. ",
